...
struct worker //структура для работника заповедника
{
	...
	struct worker *next; //для односвязных списков
};
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	struct worker *head=NULL; //указатель на первый элемент списка
	...
	display(head); //вывести динамическую структуру
	printf("\nОБНУЛЕНИЕ ПРОГУЛОВ РАБТНИКА С ДИНАМИЧЕСКОЙ СТРУКТУРОЙ\n");
	obnul(head); //обнуление прогулов
	printf("\nРЕЗУЛЬТАТ ОБНУЛЕНИЯ:\n");
	display(head); //вывод
	free(head); //очистить память от динамической структуры
