...
public: //открытые методы
	...
	void ZarplChange() //изменение зарплаты (прикладное) //для лаб 11
	{
		...
		{
			for(int i=0; i<area; i++) //для лаб 11
			{
				for(int j=0; j<kolvow; j++)
				{
					workersa[i][j].set_z(workersa[i][j].get_z()+izm); //добавить изменение к текущему
				}
			}
		}
	}
